...
void clearRegisters() {
    ...
    // clear/disable defaulr arduino ide settings for ctrlb 
	ADC->CTRLB.reg = 0; 	
    while (REG_ADC_STATUS & ADC_STATUS_SYNCBUSY);
}
...
--> Sample rate =  8 000 000/(16*8*5) = 12500 KSPS
...
void aDCSetup() {
  	...
  	// average control 8 sample
  	REG_ADC_AVGCTRL |= ADC_AVGCTRL_SAMPLENUM_8;

  	// Sampling time, no extra sampling half clock-cycles
    // by the documentation the time we need for sampling is (samplen + 1) * (cycles of mode(in free mode without gain is ~ 8)/2) so actually 5, 
    // this we use in our formula for selecting clock and divide factor clock/(divide factor*avg samples*5) >= 8 kHz, but no too big
